...
class StronglyAnnotatedSet(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.return_filename:
            ...
            idx = item.clone().detach() if type(item) == torch.Tensor else torch.tensor(item)
            out_args.append(idx)
            out_args.append(torch.tensor(True)) # synth set
...
class WeakSet(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.return_filename:
            ...
            idx = item.clone().detach() if type(item) == torch.Tensor else torch.tensor(item)
            out_args.append(idx)
            out_args.append(torch.tensor(False)) # not synth set
