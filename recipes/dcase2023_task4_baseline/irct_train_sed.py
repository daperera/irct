...
from recipes.dcase2023_task4_baseline.local.utils import calculate_macs_airct as calculate_macs


from desed_task.dataio import ConcatDatasetBatchSampler
from desed_task.dataio.irct_datasets import StronglyAnnotatedSet, UnlabeledSet, WeakSet
from desed_task.nnet.IRCT import CRNN
from desed_task.utils.encoder import ManyHotEncoder
from desed_task.utils.schedulers import ExponentialWarmup

from local.classes_dict import classes_labels
from local.irct_sed_trainer import SEDTask4
from recipes.dcase2023_task4_baseline.local.resample_folder import resample_folder, resample_audioset_folder
from recipes.dcase2023_task4_baseline.local.utils import generate_tsv_wav_durations, get_sample_distribution, flatten_list
...
def resample_data_generate_durations(config_data, test_only=False, evaluation=False):
    if not test_only:
        dsets = [
            ("synth_folder", "synth_tsv"),
            ("synth_val_folder", "synth_val_tsv" ),
            ("strong_folder", "strong_tsv"),
            ("weak_folder", "weak_tsv"),
            ("unlabeled_folder", "unlabeled_tsv"),
            ("test_folder", "test_tsv"),
            #("audioset_folder", "audioset_strong_tsv"),
        ]
    elif not evaluation:
        dsets = [("test_folder", "test_tsv")]
    else:
        dsets = [("eval_folder", "eval_tsv")]

    for (dset, tsv) in dsets:
        # if this is a desed-like dataset
        if (dset + "_44k") in config_data:
            resample_folder(
                config_data[dset + "_44k"], config_data[dset], target_fs=config_data["fs"]
            )
        # if this is a audioset-like dataset
        else:
            resample_audioset_folder(
                config_data[tsv], config_data[dset], target_fs=config_data["fs"]
            )

    if not evaluation:
        for base_set in ["synth_val", "test"]:
            if not os.path.exists(config_data[base_set + "_dur"]):
                print("generate tsv and wav durations for " + base_set)
...
def single_run(
    ...
    if test_state_dict is None:
        ...
        audioset_strong_df = pd.read_csv(config["data"]["audioset_strong_tsv"], sep="\t")
        audioset_strong = StronglyAnnotatedSet(
            config["data"]["audioset_folder"],
            audioset_strong_df,
            encoder,
            pad_to=config["data"]["audio_max_len"],
        )

        audioset_weak_df = pd.read_csv(config["data"]["audioset_weak_tsv"], sep="\t")
        audioset_weak = WeakSet(
            config["data"]["audioset_folder"],
            audioset_weak_df,
            encoder,
            pad_to=config["data"]["audio_max_len"],
        ...
        # load dataset specified by config
        name_dict = {
            "synthetic": (synth_set, synth_df),
            "weak": (weak_set, weak_df),
            "unlabeled": (unlabeled_set, None),
            "strong": (strong_set, strong_df),
            "audioset_strong": (audioset_strong, audioset_strong_df),
            "audioset_weak": (audioset_weak, audioset_weak_df),  
        }
        tot_train_data, tot_train_df = [], []
        for dataset_type in flatten_list(config["training"]["batch_type"]):
            tot_train_data.append(name_dict[dataset_type][0])
            tot_train_df.append(name_dict[dataset_type][1])
        train_dataset = torch.utils.data.ConcatDataset(tot_train_data)

        # load bach size for each dataset specified by config
        batch_sizes = flatten_list(config["training"]["batch_size"])

        # balance samplers according to class distribution
        print("estimating sample distribution for each dataset...")
        samplers = []
        label_distribution = config["training"]["label_distribution"]
        for dataset, df in zip(tot_train_data, tot_train_df):
            if df is not None and label_distribution is not None:
                data_weight = get_sample_distribution(df, target_label_distribution=label_distribution, log=False)
                weights = [data_weight[i] for i in range(len(dataset))]
                samplers.append(torch.utils.data.WeightedRandomSampler(torch.DoubleTensor(weights), int(len(dataset)), replacement=True)) 
            else:
                samplers.append(torch.utils.data.RandomSampler(dataset))
        ...
        if "epoch_len" in config["training"]:
            ...
            epoch_len = min(
                [
                    ...
                    // (
                        flatten_list(config["training"]["batch_size"])[indx]
    ...
    if fast_dev_run:
        ...
        if "epoch_len" in config["training"]:
            limit_train_batches = config["training"]["epoch_len"]
    ...
    # adapt latent_index if the model does not handle latent_index
    if "latent_index" not in config["net"]:
        for k in list(test_state_dict.keys()):
            for layer in range(0,4):
                if f".c.{layer}" in k:
                    new_k = k.replace(f".c.{layer}", f".c.{layer+7}")
                    test_state_dict[new_k] = test_state_dict[k]
                    del test_state_dict[k]
