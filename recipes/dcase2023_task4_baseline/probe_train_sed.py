...
from desed_task.nnet.probeIRCT import CRNN as CRNN
from desed_task.nnet.analyzerCRNN import CRNN as NEW_CRNN
from desed_task.utils.encoder import ManyHotEncoder
from desed_task.utils.schedulers import ExponentialWarmup

from local.classes_dict import classes_labels
from recipes.dcase2023_task4_baseline.local.probe_sed_trainer import SEDTask4
...
def single_run(
    ...
    pretrained_state_dict = torch.load(config["net"]["pretrained_path"])["state_dict"]
    
    # if ckpt is a baseline CRNN model
    if "sed_student.cnn.cnn.conv0.weight" in pretrained_state_dict.keys():
        pretrained_crnn = NEW_CRNN(**config["net"])
        updated_state_dict = {k:v for k,v in pretrained_state_dict.items() if "sed_student" in k and "total_ops" not in k and "total_params" not in k}
        

        for (k1, v1), (k2, v2) in zip(pretrained_crnn.state_dict().items(), list(updated_state_dict.items())):
            del updated_state_dict[k2]
            updated_state_dict[k1] = v2
        
        pretrained_crnn.load_state_dict(updated_state_dict)
        pretrained_crnn = pretrained_crnn.crnn

    # if ckpt is a AIRCT model
    else:
        
        pretrained_crnn = NEW_CRNN(**config["net"])
        ckpt_k_list = [k for k in pretrained_state_dict if "sed_student" in k and "total_ops" not in k and "total_params" not in k]
        ckpt_k_list = [k for k in ckpt_k_list if "e_i." in k or "c." in k]
        updated_state_dict = {k:pretrained_state_dict[k]for k in ckpt_k_list}
    ...
    # adapt to new format
    sed_student = CRNN(**config["net"], pretrained_crnn=pretrained_crnn)
