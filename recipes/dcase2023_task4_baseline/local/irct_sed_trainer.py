...
from desed_task.dataio.irct_augmentation import RandAugment
...
from desed_task.evaluation.irct_evaluation_measures import (
...
class SEDTask4(pl.LightningModule):
    ...
    def __init__(
        ...
        elif hparams["augs"]["consis_loss"] == "mse": # default value
        ...
        self.augmentor = RandAugment(self.hparams, device="cuda")
        ...
        # recover batch structure
        batch_size = np.sum(self.hparams["training"]["batch_size"])
        strong_size = np.sum(self.hparams["training"]["batch_size"][0])
        weak_size = np.sum(self.hparams["training"]["batch_size"][1]) if len(self.hparams["training"]["batch_size"]) > 1 else 0
        unlabeled_size = np.sum(self.hparams["training"]["batch_size"][2]) if len(self.hparams["training"]["batch_size"]) > 2 else 0
        batch_structure = [
            (batch_size, "original"),
            (strong_size, "mixup"),
            (strong_size, "augments"),
        ]
        if weak_size > 0:
            batch_structure += [(weak_size, "mixup"), (weak_size, "augments")]
        if unlabeled_size > 0:
            batch_structure += [(unlabeled_size, "mixup"), (unlabeled_size, "augments")]
            
        # build mixup & augmentation mask
        index = 0
        self.original_mask, self.mixup_mask, self.augments_mask = torch.zeros(batch_size*3, dtype=torch.bool), torch.zeros(batch_size*3, dtype=torch.bool), torch.zeros(batch_size*3, dtype=torch.bool)
        for (partition_size, partition_type) in batch_structure:
            if partition_type == "original":
                self.original_mask[index:index+partition_size] = 1
            elif partition_type == "mixup":
                self.mixup_mask[index:index+partition_size] = 1
            elif partition_type == "augments":
                self.augments_mask[index:index+partition_size] = 1
    ...
    def detect(self, mel_feats, model, augmentation_dict, label_partition_index):
        # handle branch processing
        mel_feats1, mel_feats2, mel_feats3 = mel_feats
        mel_feats1, mel_feats2, mel_feats3 = self.scaler(self.take_log(mel_feats1)), self.scaler(self.take_log(mel_feats2)), self.scaler(self.take_log(mel_feats3))
        mel_feats = (mel_feats1, mel_feats2, mel_feats3)
        
        # apply model
        return model(mel_feats, augmentation_dict, label_partition_index)
    ...
    def training_step(self, batch, batch_indx):
        ...
        # reformat distortion dict
        augmentation_dict = {}
        name_dict = {"weak": "weak", "synthetic": "strong", "audioset_strong": "strong", "unlabeled": "unsup"}
        for partition_name in self.hparams["training"]["batch_type"]:
            augmentation_name, augmentation_arg = distortions[name_dict[partition_name]]["augments"]
            augmentation_dict[partition_name] = {
                "mixup_mask" : masks[name_dict[partition_name]][self.mixup_mask],
                "augments_mask" : masks[name_dict[partition_name]][self.augments_mask],
                "mixup" : distortions[name_dict[partition_name]]["mixup"][1], 
                "time_shift" : augmentation_arg if augmentation_name == "Time shift" else None,
            }
        
        # format model input & label
        features1, features2, features3 = features[self.original_mask], features[self.mixup_mask], features[self.augments_mask]

        # randomly force prediction on the original, mixup or augmented partition (or all of them)
        all_partition = self.hparams["augs"]["all_partition"] if "all_partition" in self.hparams["augs"] else False
        if not all_partition:
            label_partition_index, label_partition = random.choice([(0, self.original_mask), (1, self.mixup_mask), (2, self.augments_mask)])
            labels = labels[label_partition]
            strong_mask, weak_mask = strong_mask[label_partition], weak_mask[label_partition]
        else:
            label_partition_index = -1
        labels_weak = (torch.sum(labels, -1) > 0).float()

        # sed student forward
        strong_preds_student, weak_preds_student, loss_dict_student = self.detect(
            (features1, features2, features3), self.sed_student, augmentation_dict, label_partition_index
        )
        # supervised loss on strong labels
        loss_strong = self.supervised_loss(
            strong_preds_student[strong_mask], labels[strong_mask]
        )
        # supervised loss on weak labels
        loss_weak = self.supervised_loss(weak_preds_student[weak_mask], labels_weak[weak_mask])

        # total supervised loss
        tot_loss_supervised = loss_strong + loss_weak

        with torch.no_grad():
            strong_preds_teacher, weak_preds_teacher, loss_dict_teacher = self.detect(
                (features1, features2, features3), self.sed_teacher, augmentation_dict, label_partition_index
            )
            ...
            loss_weak_teacher = self.supervised_loss(
                weak_preds_teacher[weak_mask], labels_weak[weak_mask]
            )
        ...
        # take additional losses into account
        for loss_name, loss_value in loss_dict_student.items():
            self.log("train/student/{}".format(loss_name), loss_value)
            tot_loss += (loss_value * weight) # rampup
        for loss_name, loss_value in loss_dict_teacher.items():
            self.log("train/teacher/{}".format(loss_name), loss_value)
        ...
        # also log energy consumption for debugging purposes
        training_kwh = self.tracker_train._total_energy.kWh
        self.log("/train/tot_energy_kWh", torch.tensor(float(training_kwh)), prog_bar=True)
    ...
    def validation_step(self, batch, batch_indx):
        ...
        audio, labels, padded_indxs, filenames, indxs, mask_synth = batch


        # format model input & label (force prediction on original data)
        features = self.mel_spec(audio)


        # sed student & teacher forward
        strong_preds_student, weak_preds_student, loss_dict_student = self.detect(
            (features, features, features), self.sed_student, None, 0
        )
        strong_preds_teacher, weak_preds_teacher, loss_dict_teacher = self.detect(
                (features, features, features), self.sed_teacher, None, 0
            )
        mask_weak = ~mask_synth
        ...
        for loss_name, loss_value in loss_dict_student.items():
            self.log(f"val/synth/student/{loss_name}", loss_value)
            self.log(f"val/weak/student/{loss_name}", loss_value)
        for loss_name, loss_value in loss_dict_teacher.items():
            self.log(f"val/synth/teacher/{loss_name}", loss_value)
            self.log(f"val/weak/teacher/{loss_name}", loss_value)
    ...
    def validation_epoch_end(self, outputs):
        ...
        obj_metric = torch.tensor(weak_student_f1_macro.mean() + synth_metric)

        self.log("val/obj_metric", obj_metric, prog_bar=True)
        self.log("val/weak/student/macro_F1", weak_student_f1_macro.mean())
        self.log("val/weak/teacher/macro_F1", weak_teacher_f1_macro.mean())
        ...
        self.log("val/current_epoch", self.current_epoch)
    ...
    def test_step(self, batch, batch_indx):
        ...
        self.get_weak_student_f1_seg_macro(
            weak_preds_student, labels_weak.long()
        )
        self.get_weak_teacher_f1_seg_macro(
            weak_preds_teacher, labels_weak.long()
        )

        # log remaining losses
        for loss_name, loss_value in loss_dict_student.items():
            self.log(f"test/student/{loss_name}", loss_value)
        for loss_name, loss_value in loss_dict_teacher.items():
            self.log(f"test/teacher/{loss_name}", loss_value)

    def on_test_epoch_end(self):
        # pub eval dataset
        #save_dir = os.path.join(self.exp_dir, "metrics_test")

        weak_student_f1_classwise = self.get_weak_student_f1_seg_macro.compute()
        weak_teacher_f1_classwise = self.get_weak_teacher_f1_seg_macro.compute()
        
        # format weak score
        weak_student_f1_classwise = {k:v.item() for k,v in zip(self.encoder.labels, weak_student_f1_classwise)}
        weak_teacher_f1_classwise = {k:v.item() for k,v in zip(self.encoder.labels, weak_teacher_f1_classwise)}
        weak_student_f1_macro = sum(weak_student_f1_classwise.values())/len(self.encoder.labels)
        weak_teacher_f1_macro = sum(weak_teacher_f1_classwise.values())/len(self.encoder.labels)
        

        if False:
            # remove event if null duration
            for score_buffer in [self.test_scores_raw_buffer_student, 
                                 self.test_scores_postprocessed_buffer_student, 
                                 self.test_scores_raw_buffer_teacher, 
                                 self.test_scores_postprocessed_buffer_teacher]:
                for filename, score in score_buffer.items():
                    score_buffer[filename] = score[score["onset"]<score["offset"]]
            ...
            event_macro_student, _, _, _, event_classwise_student = log_sedeval_metrics(
                self.decoded_student_05_buffer,
                self.hparams["data"]["test_tsv"],
                #os.path.join(save_dir, "student"),
            )

            event_macro_teacher, _, _, _, event_classwise_teacher = log_sedeval_metrics(
            ...
            # synth dataset
            intersection_f1_macro_student, intersection_f1_classwise_student = compute_per_intersection_macro_f1(
            ...
            # synth dataset
            intersection_f1_macro_teacher, intersection_f1_classwise_teacher = compute_per_intersection_macro_f1(
            ...
            results = {
                ...
                "test/student/weak_f1_macro": weak_student_f1_macro,
                "test/teacher/weak_f1_macro": weak_teacher_f1_macro,
                "test/student/weak_f1": weak_student_f1_classwise,
                "test/teacher/weak_f1": weak_teacher_f1_classwise,
                "test/student/event_f1_classwise": event_classwise_student,
                "test/student/intersection_f1_classwise": intersection_f1_classwise_student,
                "test/teacher/event_f1_classwise": event_classwise_teacher,
                "test/teacher/intersection_f1_classwise": intersection_f1_classwise_teacher,
            }      
