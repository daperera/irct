...
def log_sedeval_metrics(predictions, ground_truth, save_dir=None):
    ...
    return (
        ...
        {k: v["f_measure"]["f_measure"] for k,v in event_res.results()["class_wise"].items()}, # score by class
    )  # return also segment measures

def log_sedeval_metrics_from_df(predictions, gt, save_dir=None):
    """ Return the set of metrics from sed_eval
    Args:
        predictions: pd.DataFrame, the dataframe of predictions.
        ground_truth: pd.DataFrame, the dataframe of groundtruth.
...
def calculate_macs(model, config, dataset=None):
    ...
    input = (torch.randn(input_size), torch.randn(input_size)) # be careful to duplicate input to mimick branch processing
...
def calculate_macs_aircac(model, config, dataset=None):
    """
    The function calculate the multiply–accumulate operation (MACs) of the model given as input. 
    ...
    n_frames = int(((config["feats"]["sample_rate"] * config["data"]["audio_max_len"]) / config["feats"]["hop_length"])+1)
    input_size = [1, config["feats"]["n_mels"], n_frames]
    strong_label_size = [1, 10, 156]
    weak_label_size = [1, 10]
    input = (torch.randn(input_size), torch.randn(input_size)) # be careful to triplicate input to mimick rct processing
    label = ((torch.randn(weak_label_size), torch.randn(strong_label_size)), (torch.randn(weak_label_size), torch.randn(strong_label_size)))
...
def calculate_macs_airct(model, config, dataset=None):
    """
    The function calculate the multiply–accumulate operation (MACs) of the model given as input. 
    ...
    n_frames = int(((config["feats"]["sample_rate"] * config["data"]["audio_max_len"]) / config["feats"]["hop_length"])+1)
    input_size = [1, config["feats"]["n_mels"], n_frames]
    input = ((torch.randn(input_size), torch.randn(input_size), torch.randn(input_size)),  torch.randn(1))
...
def _df_to_matrix(df):
    ...
    if "event_labels" in df: 
        label_list = sorted(list(set(",".join(df.event_labels.unique()).split(","))))
        i_dict = {label: n for n, label in enumerate(label_list)}
        Y = torch.zeros((len(df), len(label_list)))
        for n, row in tqdm(df.iterrows(), total=len(df)):
            for label in row["event_labels"].split(","):
                Y[n][i_dict[label]] = 1
    # handle strong labels
    else: 
        label_list = sorted(df.event_label.unique())
        i_dict = {label: n for n, label in enumerate(label_list)}
        filename_df = df.filename.unique()
        Y = torch.zeros((len(filename_df), len(label_list)))
        for n, filename in tqdm(enumerate(filename_df), total=len(filename_df)):
            for _, row in df[df.filename==filename].iterrows():
                if row["event_label"] in label_list:
                    Y[n][i_dict[row["event_label"]]] = 1
    return Y


# default : target distribution is uniform
def get_sample_distribution(df, target_label_distribution=None, log=False):
    # init
    Y = _df_to_matrix(df)
    sample_distribution = torch.nn.Embedding(1, Y.shape[0])
    target_distribution = torch.tensor(target_label_distribution) if target_label_distribution else torch.ones(Y.shape[1]) / Y.shape[1]
    metric = torch.nn.MSELoss()
    optimizer = torch.optim.Adam(sample_distribution.parameters(), 1e-3, betas=(0.9, 0.999))
    n_epoch = 10000
    log_dict = {"loss":[], "distribution":[]}
    
    
    # gradient descent
    for _ in tqdm(range(n_epoch)):
        # compute loss
        label_distribution = (torch.nn.functional.softmax(sample_distribution.weight, dim=1) @ Y)[0]
        loss = metric(label_distribution, target_distribution)
        ...
        if log:
            ...
            log_dict["distribution"].append(label_distribution.detach().numpy())
    
    # return
    sample_distribution = torch.nn.functional.softmax(sample_distribution.weight.detach(), dim=-1)
    return (sample_distribution[0], log_dict) if log else sample_distribution[0]
...
def convert_model(path1, target_type="airct"):
    pass