...
class SEDTask4(pl.LightningModule):
    ...
    def training_step(self, batch, batch_indx):
        ...
        indx_synth = np.sum(self.hparams["training"]["batch_size"][0])
        indx_weak = np.sum(self.hparams["training"]["batch_size"][1]) if len(self.hparams["training"]["batch_size"]) > 1 else 0
        ...
        weak_mask[indx_synth:indx_synth+indx_weak] = 1 # weak_mask[:indx_weak + indx_synth] = 1
    ...
    def validation_step(self, batch, batch_indx):
        ...
        #mask_synth = torch.tensor([i<len(self.valid_data.datasets[0]) for i in indxs])
